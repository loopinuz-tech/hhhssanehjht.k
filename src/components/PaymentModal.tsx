...
import { ArrowUpRight, Wallet, X, Copy, Check, Upload, CreditCard, ShieldCheck, Clock, FileText, CheckCircle2, XCircle, Send, Zap, AlertCircle } from "lucide-react";
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span
                            className={`text-[11px] font-extrabold px-2.5 py-0.5 rounded-full flex items-center gap-1.5 border ${
                              status === "approved"
                                ? "bg-emerald-50/90 text-emerald-600 border-emerald-200/50 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20"
                                : status === "rejected"
                                ? "bg-rose-50/90 text-rose-600 border-rose-200/50 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20"
                                : "bg-amber-50/90 text-amber-600 border-amber-200/50 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20"
                            }`}
                          >
                            {status === "approved" ? (
                              <>
                                <CheckCircle2 className="w-3.5 h-3.5 opacity-70" /> Tasdiqlandi
                              </>
                            ) : status === "rejected" ? (
                              <>
                                <XCircle className="w-3.5 h-3.5 opacity-70" /> Rad etildi
                              </>
                            ) : (
                              <>
                                <Clock className="w-3.5 h-3.5 opacity-70 animate-pulse" /> Kutilmoqda
                              </>
                            )}
                          </span>
                        </div>
                        ...
                        {status === "pending" && (
                          <p className="text-[11px] text-amber-700 dark:text-amber-400 bg-amber-50/60 dark:bg-amber-500/10 p-2.5 rounded-lg border border-amber-200/50 dark:border-amber-500/20 flex items-start gap-1.5 font-medium leading-relaxed">
                            <Clock className="w-3.5 h-3.5 text-amber-500 shrink-0 mt-0.5 opacity-75" />
                            <span>Admin to'lov so'rovingizni tekshirib balansingizni to'ldiradi.</span>
                          </p>
                        )}

                        {status === "approved" && (
                          <p className="text-[11px] text-emerald-700 dark:text-emerald-400 bg-emerald-50/60 dark:bg-emerald-500/10 p-2.5 rounded-lg border border-emerald-200/50 dark:border-emerald-500/20 flex items-start gap-1.5 font-medium leading-relaxed">
                            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0 mt-0.5 opacity-75" />
                            <span>To'lov admin tomonidan tasdiqlandi va balansingizga o'tkazildi!</span>
                          </p>
                        )}

                        {status === "rejected" && (
                          <p className="text-[11px] text-rose-700 dark:text-rose-400 bg-rose-50/60 dark:bg-rose-500/10 p-2.5 rounded-lg border border-rose-200/50 dark:border-rose-500/20 flex items-start gap-1.5 font-medium leading-relaxed">
                            <AlertCircle className="w-3.5 h-3.5 text-rose-500 shrink-0 mt-0.5 opacity-75" />
                            <span>To'lov cheki rad etildi. Ma'lumotlarni qayta tekshirib yangi so'rov yuboring.</span>
                          </p>
                        )}
                      </div>
                    );
