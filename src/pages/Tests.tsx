...
const Tests = () => {
  ...
  // Fetch setting to hide empty ("Tez orada") tests set by Admin in AdminCatalog
  const { data: hideEmptyTests = false } = useQuery({
    queryKey: ["catalog-hide-empty-tests"],
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from("admin_settings")
        .select("value")
...
function getSubjectIcon(name: string): any {
  ...
  const filteredItems = useMemo(() => {
    ...
    // Hide empty ("Tez orada") tests if admin setting is enabled
    if (hideEmptyTests) {
      items = items.filter((i: any) => (i.questions_count || 0) > 0);
    }

    return items;
  }, [folders, userTests, mockTests, activeCategory, activeSubject, searchTerm, questionCountsMap, hideEmptyTests]);
  ...
  const subjectCounts = useMemo(() => {
    ...
    if (hideEmptyTests) {
      items = items.filter((item: any) => (item.questions_count || 0) > 0);
    }
    ...
    return counts;
  }, [folders, mockTests, userTests, activeCategory, hideEmptyTests]);
